<template>
    <div id="board">
        <MessageDisplay v-if="trains.length === 0">No trains arriving :(</MessageDisplay>
        <TrainDisplay v-if="trains.length > 0" lineNumber="1" :destination="trains[0].destination" :eta="trains[0].eta"/>
        <transition name="slide-up">
            <TrainDisplay key="lineTwo" v-if="lineTwoTrain && !showLineThree" :lineNumber="lineTwoIndex + 1" :destination="lineTwoTrain.destination" :eta="lineTwoTrain.eta"/>
            <TrainDisplay key="lineThree" v-if="lineTwoTrain && showLineThree" :lineNumber="lineTwoIndex + 1" :destination="lineTwoTrain.destination" :eta="lineTwoTrain.eta"/>
        </transition>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            cycleDisplay() {
                let originalIndex = this.lineTwoIndex;
        ...
        mounted() {
            this.getTrains();
            setInterval(this.cycleDisplay, 10000);
            setInterval(this.getTrains, 45000);
        }
    }
</script>